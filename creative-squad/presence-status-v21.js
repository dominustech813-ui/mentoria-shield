(()=>{
  ...
  async function installSidebarPanel(){
    ...
    const name=q('cdlPresenceName'),img=q('cdlPresenceAvatar');
    const newName=profile.display_name||profile.nickname||'Membro CDL';
    if(name&&name.textContent!==newName)name.textContent=newName;
    if(img){const src=await avatarFor(profile);if(img.src!==src)img.src=src}
    renderOwnStatus();renderMenu();
  }

  function renderOwnStatus(){
    const s=getOwnStatus(),m=meta(s),dot=q('cdlPresenceDot'),label=q('cdlPresenceLabel');
    if(dot&&dot.className!=='cdlPresenceDot '+m.cls)dot.className='cdlPresenceDot '+m.cls;
    if(label&&label.textContent!==m.label)label.textContent=m.label;
    const select=q('cdlProfilePresenceSelect');if(select&&select.value!==s)select.value=s;
    document.querySelectorAll('.cdlPresenceOption').forEach(b=>b.classList.toggle('active',b.dataset.status===s));
    const selfDot=document.querySelector('#voiceDock .dvOnline');if(selfDot)selfDot.className='dvOnline '+m.cls;
  }

  function renderMenu(){
    const menu=q('cdlPresenceMenu');if(!menu)return;
    if(menu.dataset.ready==='1'){renderOwnStatus();return}
    const opts=['online','idle','dnd','invisible'];
    menu.innerHTML=opts.map(s=>{const m=meta(s);return `<button class="cdlPresenceOption" data-status="${s}" type="button" onclick="CDLPresence.setStatus('${s}')"><span class="cdlStatusIcon ${m.cls}"></span><span><strong>${m.label}</strong><small>${m.desc}</small></span></button>`}).join('');
    menu.dataset.ready='1';renderOwnStatus();
  }
  ...
  async function setStatus(status){
    ...
    profile={...profile,...data};try{profiles.set(data.id,{...(profiles.get(data.id)||{}),...data})}catch{}
    renderOwnStatus();applyVoiceStatuses();
    q('cdlPresenceMenu')?.classList.add('hidden');
  }
  ...
  function toggleMenu(force){const menu=q('cdlPresenceMenu');if(!menu)return;const show=typeof force==='boolean'?force:menu.classList.contains('hidden');menu.classList.toggle('hidden',!show)}
  ...
  async function start(){
    ...
    sub=sb.channel('cdl-presence-v21-'+Date.now()).on('postgres_changes',{event:'UPDATE',schema:'public',table:'profiles'},payload=>{
  ...
  try{const oldEnter=enter;enter=function(){const out=oldEnter();setTimeout(start,120);return out}}catch{}
  try{const oldRenderProfile=renderProfile;renderProfile=async function(){const out=await oldRenderProfile();installProfileControl();await installSidebarPanel();renderOwnStatus();return out}}catch{}
  try{const oldRenderVoices=renderVoices;renderVoices=function(){const out=oldRenderVoices();setTimeout(applyVoiceStatuses,120);return out}}catch{}
  ...
  setInterval(()=>{if(user&&!started)start()},1000);
})();